function parametersHR = dLoad_HRsettings_DT07B

%%% Filter and FFT params %%
parametersHR.bpRanges = [15000,48000]; % Bandpass filter params in Hz [min,max]
parametersHR.frameLengthUs = 2048; % For fft computation
parametersHR.overlap = .5; % FFT overlap (in decimal, not percent form)
parametersHR.chan = 4; % which channel do you want to look at?
...
parametersHR.countThresh = 2000; % Keep consistent with Lo-res for predictability.
% Can be higher than low res, but not lower!
% Keep count threshold less than equivalent pp threshold. 
%   dBs = 20*log10(abs(fft(counts *2^14))) - 10*log10(fs/(length(fftWindow)))...
%            + transfer function
% note: array uses 2^15

%%% Envelope params %%%
parametersHR.energyThr = 0.5; % n-percent energy threshold for envelope duration
parametersHR.dEvLims = [-.3,.9];  % [min,max] Envelope energy distribution comparing 
...
parametersHR.delphClickDurLims = [.2,10];% [min,max] duration in microsec 
% allowed for high energy envelope of click


%%% Other pruning params %%%
parametersHR.cutPeakBelowKHz = 14; % discard click if peak frequency below X kHz
...
parametersHR.maxClick_us = 3000; % Max duration of a click including echos
parametersHR.maxNeighbor = 12; % max time in seconds allowed between neighboring 
