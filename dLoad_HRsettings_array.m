function parametersHR = dLoad_HRsettings_array

%%% Filter and FFT params %%
parametersHR.bpRanges = [10000,90000]; % Bandpass filter params in Hz [min,max]
parametersHR.frameLengthUs = 2000;%1024;%(HARP); % For fft computation
parametersHR.overlap = .5; % FFT overlap (in decimal, not percent form)
parametersHR.chan = 2; % which channel do you want to look at?
parametersHR.clipThreshold = .99;%  Normalized clipping threshold btwn 0 and 1.  If empty, 
% assumes no clipping.

 
%%% Recieved level threshold params %%%
parametersHR.ppThresh = 130;% minimum  RL threshold - dB peak to peak.
...
parametersHR.dEvLims = [-.9,1];  % [min,max] Envelope energy distribution comparing 
...
parametersHR.delphClickDurLims = [20,400];% [min,max] duration in microsec 
...
parametersHR.minClick_us = 1;% Minimum duration of a click in us 
parametersHR.maxClick_us = 1000; % Max duration of a click including echos
parametersHR.maxNeighbor = 100; % max time in seconds allowed between neighboring 
...
parametersHR.mergeThr = 20;% min gap between energy peaks in us. Anything less
% will be merged into one detection. If the beginning of the next is fewer
